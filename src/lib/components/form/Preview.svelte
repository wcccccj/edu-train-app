<script lang="ts">
	import type { SectionedFormSchema, FormValues, FormField, FormFieldOption } from './form.types';
	...
	function getField(fieldName: string) {
		for (const section of schema.sections) {
			const field = section.fields.find((f: FormField) => f.name === fieldName);
	...
	function getDisplayValue(fieldName: string) {
		...
		if (field.type === 'select' && field.options) {
			return (
				field.options.find((o: FormFieldOption) => o.value === data[fieldName])?.label ||
				data[fieldName]
			);
		}
